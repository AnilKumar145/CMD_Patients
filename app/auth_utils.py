...
SECRET_KEY = "your_secret_key_here_change_in_production"  # Update this to match auth microservice
ALGORITHM = "HS256"
...
async def get_current_user(
    ...
    if security_scopes.scopes:
        ...
        if not has_required_scope:
            logger.error(f"User role {token_data.role} not in required scopes: {security_scopes.scopes}")
            raise HTTPException(
    ...
    logger.info(f"User authenticated: {user.username} with role {user.role}")
    return user
